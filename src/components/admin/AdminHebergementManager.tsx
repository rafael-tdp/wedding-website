 ...
 */
export default function AdminHebergementManager({ showCreateForm, setShowCreateForm, showEditForm, setShowEditForm }: { showCreateForm: boolean; setShowCreateForm: (show: boolean) => void; showEditForm: boolean; setShowEditForm: (show: boolean) => void }) {
  ...
  return (
    <>
      {/* Modal pour ajouter un hébergement */}
      <AdminFormModal
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Distance/Durée de trajet</label>
            <input
              type="text"
      ...
      {/* Modal pour modifier un hébergement */}
      <AdminFormModal
        isOpen={showEditForm}
        onClose={() => {
          setEditingId(null);
      ...
      {hebergements.length === 0 ? (
        ...
        <div className="space-y-4">
          {hebergements.map((hebergement) => (
            ...
            <div key={hebergement.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow p-4 sm:p-6">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-2 sm:gap-4">
                    <div className="flex-1">
                      ...
                      <div className="flex flex-col gap-1 mt-2 text-xs sm:text-sm text-gray-600">
                        {hebergement.type && (
                          <span><span className="font-medium text-gray-700">Type:</span> {hebergement.type === 'hotel' ? 'Hôtel' : 'Airbnb'}</span>
                        )}
                        {hebergement.price && (
                          <span><span className="font-medium text-gray-700">Prix:</span> {hebergement.price}€</span>
                        )}
                        {hebergement.website && (
                          <span><span className="font-medium text-gray-700">Site: </span> <a href={hebergement.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 break-all">Cliquer pour visiter</a></span>
                        )}
                        {hebergement.length && (
                          <span><span className="font-medium text-gray-700">Trajet:</span> {hebergement.length} min</span>
                        )}
                      </div>
                    </div>
